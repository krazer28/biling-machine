...
(function () {
  ...
  const CONFIG = {
    ...
    GOOD_MATCH_DISTANCE: 40, // stricter: ORB/Hamming distance below which a match counts as "good" (0-256 scale)
    MIN_GOOD_MATCHES: 30, // stricter: minimum good matches required to accept a recognition
    CONFIDENCE_DISPLAY_SCALE: 40, // good-match count that maps to ~100% on the confidence readout
    MIN_CONFIDENCE_PERCENT: 75, // extra guard: reject even a "best" match if displayed confidence is still low
    ORB_FEATURES: 1000,
  };
  ...
  function matchFrameAgainstReferences(frameGray) {
    ...
    // Reject weak matches completely — the scanner must be allowed to
    // return "no product" rather than guessing from the best of a bad lot.
    if (!best) return null;

    const confidencePercent = Math.min(100, Math.round((best.goodMatches / CONFIG.CONFIDENCE_DISPLAY_SCALE) * 100));

    // Strict validation: both the raw match count AND the displayed
    // confidence must clear their thresholds.
    if (best.goodMatches < CONFIG.MIN_GOOD_MATCHES || confidencePercent < CONFIG.MIN_CONFIDENCE_PERCENT) {
      return null;
    }
